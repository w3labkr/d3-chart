...
(function ($, window, document, undefined) {
  ...
  ChartPie.defaults = {
    ...
    // Legend2nd
    legend2ndMarginTop: 10,
    legend2ndMarginLeft: 10,
    legend2ndDirection: "vertical", // (string) horizontal, vertical
    legend2ndHorizontalGutter: 10,
    legend2ndVerticalGutter: 10,
    legend2ndFontFamily: "sans-serif",
    legend2ndFontSize: "12px",
    legend2ndColor: "#757575",
    legend2ndIconSize: 12,
    legend2ndTextX: 15,
    legend2ndTextY: 10,
  };
  ...
  ChartPie.prototype.onInit = function (element, settings) {
    ...
    (dataset = hasInitialData ? settings.initialData : settings.data),
      (yDomainMarginTop = hasInitialData
        ? settings.initialyDomainMarginTop
        : settings.yDomainMarginTop);

    var keys = !!dataset.length ? Object.keys(dataset[0]) : null;
    ...
    var pie = d3
      ...
      //         case 'ascending' : return d3.ascending(objA, objB);
      //         case 'descending': return d3.descending(objA, objB);
      //         case '': return null;
      //     }
      //     return null;
      // })
     ...
     * Legend2nd
     */
    var legend2nd = function (g) {
      g.attr("font-family", settings.legend2ndFontFamily)
        .attr("font-size", settings.legend2ndFontSize)
        .attr("color", settings.legend2ndColor)
        .sort(null)
        .call(function (g) {
          g.append("rect")
            .attr("x", 0)
            .attr("y", 0)
            .attr("width", settings.legend2ndIconSize)
            .attr("height", settings.legend2ndIconSize)
            .attr("fill", function (d, i) {
              return hasInitialData ? settings.initialColor : color(i);
            });
        })
        .call(function (g) {
          g.append("text")
            .attr("x", settings.legend2ndTextX)
            .attr("y", settings.legend2ndTextY)
            .attr("font-family", settings.legend2ndFontFamily)
            .attr("font-size", settings.legend2ndFontSize)
            .attr("color", settings.legend2ndColor)
            .text(function (d) {
              return Object.values(d.data)[1] + settings.legend2ndFormat;
            });
        })
        .call(function (g) {
          var translates = [],
            ...
            translateY = 0;
          g.attr("transform", function (d, i) {
            switch (settings.legend2ndDirection) {
              case "horizontal":
                var bbox = this.getBBox();
                translates[i] =
                  parseInt(bbox.width) + settings.legend2ndHorizontalGutter;
                translateX = d3.sum(translates) - translates[i];
                // TODO: SUPPORT MULTILINE LEGEND
                return "translate(" + translateX + "," + translateY + ")";
              case "vertical":
                return (
                  "translate(0, " +
                  i * settings.legend2ndVerticalGutter * 2 +
                  ")"
                );
            }
          });
    ...
    svg
      ...
      .attr("d", arc)
      .attr("fill", function (d, i) {
        return hasInitialData ? settings.initialColor : color(d.index);
      })
    ...
    svg
      .append("g")
      .attr("class", "legend2nd")
      .attr("transform", function () {
        var legendX = -margin.left + settings.legend2ndMarginLeft,
          legendY = -margin.top + settings.legend2ndMarginTop;
        return "translate(" + legendX + "," + legendY + ")";
      })
      .selectAll("g")
      .data(pie(dataset))
